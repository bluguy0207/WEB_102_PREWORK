...
// set the inner HTML using a template literal and toLocaleString for commas
...
function filterFundedOnly() {
    ...
    // check how many games are funded (Secret Key component 2)
...
// use filter to count unfunded games
const numUnfunded = GAMES_JSON.filter(game => game.pledged < game.goal).length;
const numGames = GAMES_JSON.length;

// create a string using ternary operator
const displayStr = `A total of ${numUnfunded} game${numUnfunded === 1 ? '' : 's'} remain unfunded. 
We need your help to fund these amazing ${numGames} games!`;
